...
__device__ __host__ bool isPointOnLine(long long int xc, long long int yc, long long int x1, long long int y1, long long int x2, long long int y2) {
...
__global__ void initKernel(int H, int *device_scoring, int *device_hp){
    device_hp[threadIdx.x] = H;
    device_scoring[threadIdx.x] = 0;
...
__global__ void tankInteractionKernel(int *device_x,int *device_y,int *device_scoring,int *device_hp, int T,long long int currentRound, int *device_destroyed){
    ...
    if(!device_destroyed[blockIdx.x] && !device_destroyed[threadIdx.x]){
        if (blockIdx.x != target_id && threadIdx.x != blockIdx.x && lieOnLine && direction == dir)
        {
            do {
...
// }
__global__ void roundC(int T,int *currentRound){
    if (*currentRound % T == 0) {
        *currentRound++;
    }
}
...
int main(int argc, char **argv) {
    ...
    x = (int *)malloc(T * sizeof(int));  // X coordinate of each tank
    y = (int *)malloc(T * sizeof(int));  // Y coordinate of each tank
    score = (int *)malloc(T * sizeof(int));  // Score of each tank (ensure that at the end you have copied back the score calculations on the GPU back to this allocation)

    // Get the Input of Tank coordinates
    for (int i = 0; i < T; i++) {
        fscanf(inputfilepointer, "%d", &x[i]);
        fscanf(inputfilepointer, "%d", &y[i]);
    }
    ...
    //     hp[i] = H;
    // }
    initKernel<<<1,T>>>(H, device_scoring, device_hp);
