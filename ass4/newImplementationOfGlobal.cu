...
#include <vector>
using namespace std;
__device__ int globalArray[1024];
__global__ void initializeArray() {
    int threadId = threadIdx.x;
    globalArray[threadId] = INT_MAX;
}
...
__global__ void tankInteractionKernel(int *device_x,int *device_y,int *device_scoring,int *device_hp, int T,int currentRound, int *device_destroyed){
    ...
    printf("Point %d target %d Distance from point %d is %d direction %d true_dir %d \n", tank_id, target_id, threadIdx.x ,distance, direction, dir);
    if(!device_destroyed[blockIdx.x] && !device_destroyed[threadIdx.x]){

        if (threadIdx.x != blockIdx.x && lieOnLine && direction == dir && distance < globalArray[blockIdx.x])
        {
            atomicMin(&globalArray[blockIdx.x], distance);
            atomicExch(&it, threadIdx.x);
            printf("Point %d target %d Distance from point %d is %d direction %d true_dir %d least_dist_currently %d \n", tank_id, target_id, threadIdx.x ,distance, direction, dir, globalArray[blockIdx.x]);
        }
