    ...
    for (int i = 0; i < T; i++) {
        ...
        if (dir == 1) {
            if (tanks.hp[i] > 0 && x1 <= tanks.x[i] && y1 <= tanks.y[i]) {
                if ((abs(x1 - tanks.x[i]) + abs(y1 - tanks.y[i])) < temp) {
...
vector<vector <int>> f(int T, Tanks tanks, int currentRound){
    vector<vector<int>> tankHitCount(2, vector<int>(T, 0));
...
int main(int argc, char **argv) {
    ...
    while (tanksLeft > 1) {  // Continue rounds until only one or zero tanks left
        ...
        for(int i = 0; i < T; ++i) {
            tanks.hp[i] -= tankHitCount[0][i];
            tanks.score[i] += tankHitCount[1][i];
            if (tanks.hp[i] <= 0 && tankHitCount[0][i]) tanksLeft--;
        }
        currentRound++;
    }
